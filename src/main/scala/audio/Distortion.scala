...
class Distortion(defaultCtrl: Int = 10) extends DSPModule(defaultCtrl) {
  ...
  val lookupValues =
    Range(lookupSteps - 1, maxGainSignal + 1, lookupSteps).map(i =>
      maxSignal * (1.0 - scala.math.exp(
        -gainConst * i.toDouble / maxSignal.toDouble
      ))
    )
  val lookupTable = VecInit(
    lookupValues.map(v => scala.math.round(v).asUInt(absDataWidth.W))
  )

  val inVal = audioInReg
  val inValAbs = inVal.abs.asUInt.min(maxSignal.U).tail(1)
  val regInValSign =
    ShiftRegister(inVal(DATA_WIDTH - 1), 2) // delay by two stages
  ...
  when(regInValSign === true.B) {
    audioOut := -(regInterp +& regLookupLow).asSInt()
  }.otherwise {
    audioOut := (regInterp +& regLookupLow).asSInt()
  }
